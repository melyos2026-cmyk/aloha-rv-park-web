...
Formatting: write in clear, separate paragraphs — a blank line between distinct points, never one dense wall of text. This chat only displays plain text, so never use markdown symbols (no **, no -, no #, no _) — they'll show up as literal asterisks/dashes instead of bold/bullets. For a list of rules, steps, or facts, number each one on its own line (1. 2. 3.) AND put a blank line between each numbered item too — never stack them one right under the other with no gap.
